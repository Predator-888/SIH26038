...
def get_case_explanation(
    ...
    """
    Returns the detailed clinical explanation and validation generated by Gemini 2.5 Flash.
    """
    ...
    try:
        val = gemini_service.validate_and_explain_case(case_id, session, lang=lang, force_refresh=False)
        return {
            "case_id": case_id,
            "status": val.status,
            "macular_edema_risk": val.macular_edema_risk,
            "clinical_recommendation": val.clinical_recommendation,
            "clinical_explanation": val.clinical_explanation,
            "model_name": val.model_name,
            "created_at": val.created_at.isoformat()
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail={"code": "EXPLANATION_ERROR", "message": str(e)})


@router.post("/{case_id}/explanation/refresh")
def refresh_case_explanation(
    case_id: str,
    lang: str = "en",
    session: Session = Depends(get_session)
):
    """
    Forces a fresh live re-query to Gemini 2.5 Flash for the case.
    """
    from backend.app.services.gemini_service import gemini_service
    case = session.get(Case, case_id)
    if not case:
    ...
    try:
        val = gemini_service.validate_and_explain_case(case_id, session, lang=lang, force_refresh=True)
        return {
            "case_id": case_id,
            "status": val.status,
            "macular_edema_risk": val.macular_edema_risk,
